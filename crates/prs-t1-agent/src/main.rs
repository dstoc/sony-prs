    ...
    match args.next().as_deref() {
        ...
        Some("render-test") => {
            ...
            if args.next().is_some() {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "render-test accepts FRAMEBUFFER, optional SECONDS, and optional WAVEFORM",
                ));
            }
            let mut stdout = io::stdout().lock();
