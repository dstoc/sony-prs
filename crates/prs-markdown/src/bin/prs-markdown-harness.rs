...
mod host {
    ...
    fn run() -> Result<(), Box<dyn Error>> {
        ...
        fs::create_dir_all(&cli.output).map_err(|error| {
            format!("create output directory {}: {error}", cli.output.display())
        })?;
        ...
        fn parse(mut args: impl Iterator<Item = String>) -> Result<Option<Self>, String> {
            ...
            while let Some(argument) = args.next() {
                match argument.as_str() {
                    ...
                    "--width" => {
                        width = parse_u32(&required_value(&mut args, "--width")?, "--width")?
                    }
                    "--height" => {
            ...
            let input =
                input.ok_or_else(|| "provide a Markdown file or --fixture NAME".to_owned())?;
            Ok(Some(Self {
                input,
                output,
                pages,
