...
described in `firmware-analysis.md`.
...
Malformed or unsupported requests return a `PRS1 ERR ...` response. The host
implementation is in `crates/prs350-wire/src/lib.rs` and
`crates/prs350-devctl/src/serial.rs`; the CLI exposes it as
`prs350-devctl ping /dev/ttyACM0`, `prs350-devctl info /dev/ttyACM0`, and
`prs350-devctl status /dev/ttyACM0`. The separate
`prs350-devctl reboot /dev/ttyACM0` command sends only the explicit reboot
request. The same tool also exposes the deliberately stateful render,
screenshot, binary-execution, and root-shell controls described below.
...
`prs350-devctl screenshot /dev/ttyACM0 screen.pgm` sends `PRS1 CAPTURE\n`. The
reader replies with a header such as:
...
`prs350-devctl exec /dev/ttyACM0 ./prs350-agent-test` first sends an ARM-binary
header and waits for the reader to switch the tty to raw mode:
...
```text
prs350-devctl screenshot /dev/ttyACM0 screen.pgm
prs350-devctl exec /dev/ttyACM0 ./prs350-agent-test
prs350-devctl shell /dev/ttyACM0 'cat /proc/cmdline'
```

`prs350-devctl shell /dev/ttyACM0 'cat /proc/cmdline'` uses the same ready/raw framing and per-chunk acknowledgements,
...
now only configures the tty and `exec`s the Rust `prs350-agent service` mode;
...
package builder copies those files and the cross-built `prs350-agent` into a
fresh directory, then invokes the externally obtained Sony signing helpers:

```text
tools/prs350/build-dev-package.sh \
  /path/to/PRS350_update_tools/login_update \
  /path/to/shadow \
  target/armv5te-unknown-linux-musleabi/release/prs350-agent \
...
```text
tools/prs350/stage-service-package.sh /tmp/prs350-dev-package /mnt/prs350-hook-test
```
